purchaserApp.controller("purchaserInventoryNewDetailUpdateCtrl", function(
    $scope, $q, $routeParams, diabloPromise, dateFilter, diabloPattern,
    diabloUtilsService, diabloFilter, wgoodService, purchaserService,
    user, filterBrand, filterFirm, filterType, filterEmployee, filterSizeGroup, filterColor){
	...
	for(var i = 0; i < length; i++){
		...
		add.amounts.push({
		    cid:invs[i].color_id, size:invs[i].size, count:invs[i].amount})
		sorts.push(add); 
	    } 
	}
	...
	purchaserService.update_w_inventory_new({
		...
		diabloUtilsService.response_with_callback(
		    ...
		    $scope, function(){diablo_goto_page("#/record/inventory_new_detail")})
	    	return;
	    } else{
	    	diabloUtilsService.response_with_callback(
	    	    false, "入库单修改",
